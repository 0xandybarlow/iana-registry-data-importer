...
export interface DatasourceConfig extends RegistryMetadata {
  registry_id: string;
  dataset_id: string;
  primary_keys?: string[];
}
...
const applyPrimaryKeys = (
  ...
  const overrides: Record<string, string[]> = {
    ...
    oauth_extensions_error_registry: ['name', 'usage_location'],
    oauth_parameters: ['name', 'parameter_usage_location'],
    ...
    json_web_key_parameters: ['parameter_name', 'used_with_kty_value'],
...
export const REGISTRIES: { name: string; sources: DatasourceConfig[] }[] = [
  {
    name: 'OAuth Registry',
